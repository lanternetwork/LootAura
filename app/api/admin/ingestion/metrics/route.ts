...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const nativeCoordBaseFilter = (q: ReturnType<typeof fromBase>) =>
      q
        .eq('source_platform', 'external_page_source')
        .is('lat', null)
        .is('lng', null)
    ...
    const nativeCoordBacklogPromise = nativeCoordBaseFilter(fromBase(admin, 'ingested_sales'))
      .select('id', { count: 'exact', head: true })
      .eq('status', 'needs_geocode')

    const nativeCoordClaimEligiblePromise = nativeCoordBaseFilter(fromBase(admin, 'ingested_sales'))
      .select('id', { count: 'exact', head: true })
