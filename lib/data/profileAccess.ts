...
async function readUserProfile(
  ...
  const fromView = await fetchProfileV2(supabase, userId)
  if (fromView) {
    return fromView
  }

  try {
    const { data: rpcData, error: rpcError } = await supabase.rpc('get_profile', {
      p_user_id: userId,
    })
    if (rpcData && !rpcError) {
...
export async function getUserProfile(
  ...
  try {
    let profile = await readUserProfile(supabase, userId)
    if (profile) {
      return profile
    }

    const ensured = await ensureLootauraProfileExists({ userId })
    if (ensured.ok) {
      profile = await readUserProfile(supabase, userId)
      if (profile) {
        return profile
      }
    }
