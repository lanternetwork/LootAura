...
DROP FUNCTION IF EXISTS public.sale_drafts_insert();
DROP FUNCTION IF EXISTS public.sale_drafts_update();
DROP FUNCTION IF EXISTS public.sale_drafts_delete();
...
RETURNS TRIGGER AS $$
BEGIN
    UPDATE lootaura_v2.sale_drafts
    SET
        title = COALESCE(NEW.title, OLD.title),
        payload = COALESCE(NEW.payload, OLD.payload),
    ...
    SELECT * INTO NEW FROM lootaura_v2.sale_drafts WHERE id = OLD.id;
