...
import { minimalMetrics } from '@/tests/unit/admin/ystmStabilizationExitCriteria.test'
...
describe('buildIngestionDiagnostics', () => {
  ...
  it('appends YSTM coverage section when ystmCoverage is provided', () => {
    ...
    expect(md).toContain('## Coverage scoreboard telemetry')
  ...
  it('includes dashboard supplements when provided', () => {
    const detailFirst = minimalMetrics().funnel['24h'].detailFirst
    const data = {
      ...minimalMetrics(),
      funnel: {
        '24h': {
    ...
    const md = buildIngestionDiagnostics(data, {
      supplements: {
        tieredScheduler: {
          enabled: true,
          enabledAt: '2026-06-14T12:00:00.000Z',
          legacyCursor: 724,
          longTailCursor: 800,
        },
        seoOperational: {
    ...
    expect(md).toContain('## Pipeline depth')
