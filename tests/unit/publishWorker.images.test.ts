...
describe('publish worker idempotent sale images', () => {
  ...
  it('on unique conflict, formats existing linked sale address when only defect is casing (dedupe unchanged)', async () => {
    const okUrl = 'https://images.example.org/address-casing-only.jpg'
    const updateSpy = vi.fn().mockReturnValue({
      eq: async () => ({ error: null }),
    })

    const lowerNoDup = '620 lincoln ave, winnetka, il 60093'
