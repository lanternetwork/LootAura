...
const reconCtx = vi.hoisted(() => ({
  ...
  ingestRow: {
    ...
    last_source_sync_at: null as string | null,
    source_sync_status: null as string | null,
    source_sync_attempt_count: 0,
    source_sync_failure_count: 0,
    source_missing_count: 0,
    source_placeholder_detected: false,
    source_content_hash: null as string | null,
    source_schedule_hash: null as string | null,
    source_image_hash: null as string | null,
    status: 'published',
    is_duplicate: false,
    last_source_change_at: null as string | null,
  },
}))
          ...
          select: () => ({
            eq: () => ({
              is: () => ({
                not: () => ({
                  or: () => ({
                    limit: () => Promise.resolve({ data: [reconCtx.saleRow], error: null }),
                  }),
                }),
              }),
            }),
      ...
      if (table === 'ingested_sales') {
        ...
        const chain = {
          ...
          not: () => {
            notCalls += 1
            if (notCalls >= 2) {
              return Promise.resolve({ data: [reconCtx.ingestRow], error: null })
            }
            return chain
          },
          update: () => ({
