/**
 * Unit tests for splash hide strategy (native-first):
 * (1) Splash hides on APP_READY if it arrives first (optional optimization).
 * (2) Otherwise splash hides after loading=false plus short delay (normal production path).
...
describe('Splash hide strategy', () => {
  ...
  it('(2) otherwise splash hides after loading=false plus short delay', () => {
    ...
    // Delay timer callback checks splashHiddenByRef before hiding
